...
class AccountInvoice(models.Model):
    ...
    def _auto_init(self):
        # Edi type document
        if not column_exists(self.env.cr, "account_invoice", "ei_type_document_id"):
            create_column(self.env.cr, "account_invoice", "ei_type_document_id", "integer")
        ...
        if not column_exists(self.env.cr, "account_invoice", "ei_amount_tax_withholding"):
            create_column(self.env.cr, "account_invoice", "ei_amount_tax_withholding", "numeric")

        if not column_exists(self.env.cr, "account_invoice", "ei_amount_tax_no_withholding"):
            create_column(self.env.cr, "account_invoice", "ei_amount_tax_no_withholding", "numeric")

        if not column_exists(self.env.cr, "account_invoice", "ei_amount_total_no_withholding"):
            create_column(self.env.cr, "account_invoice", "ei_amount_total_no_withholding", "numeric")

        if not column_exists(self.env.cr, "account_invoice", "ei_amount_excluded"):
            create_column(self.env.cr, "account_invoice", "ei_amount_excluded", "numeric")
        ...
        # Value letters
        if not column_exists(self.env.cr, "account_invoice", "value_letters"):
            create_column(self.env.cr, "account_invoice", "value_letters", "varchar")
        ...
        for invoice_line_id in self.invoice_line_ids:
            if invoice_line_id.account_id:
                ...
                if self.ei_type_document_id.id == 12:
                    if self.date_invoice != fields.Date.context_today(self):
                    ...
                    invoice_temps.update({
                        'period': {
                            'date': fields.Date.to_string(self.date_invoice),
                            'transmission_code': 1
                        }
                    })
    ...
    def should_send_document_to_dian(self):
        ...
        if self.type in ('out_invoice', 'out_refund') and type_edi_document in ('invoice', 'credit_note', 'debit_note'):
            return True
        if self.type in ('in_invoice', 'in_refund') and type_edi_document in ('doc_support', 'note_support'):
            return True
    ...
    @api.depends('ei_is_correction_without_reference', 'debit_invoice_id', 'journal_id', 'type')
    def _compute_ei_type_document(self):
        ...
        for rec in self:
            ...
            is_debit = (hasattr(rec,
                                'debit_invoice_id') and rec.debit_invoice_id) or rec.ei_is_correction_without_reference

            # Safely extract resolution type ID from journal sequence
            try:
                resolution_type_id = rec.journal_id.sequence_id.resolution_id.resolution_type_document_id.id
            ...
            if rec.type == 'in_invoice':
                ...
                type_edi_document = type_mapping.get(rec.type, 'none')
    ...
    def validate_dian_generic(self, is_test):
        for rec in self:
            try:
                ...
                _logger.debug("Failed to process the DIAN request for document: %s: %s", (rec.number, e))
                if not rec.company_id.ei_always_validate:
                    raise UserError(_("Failed to process the DIAN request for document: %s: %s") % (rec.number, e))
                else:
                    rec.message_post(body=_("DIAN Electronic invoicing: "
                                            "Failed to process the DIAN request for document: %s: %s") % (rec.number, e))
    ...
    def action_invoice_open(self):
        ...
        if to_force_edi:
            # Validate invoices
            to_force_invoices = to_force_edi.filtered(lambda inv: inv.state == 'open')
    ...
    def get_uuid_from_nimbus(self):
        for rec in self:
            ...
            try:
                if rec.should_send_document_to_dian() and rec.type in ('in_invoice', 'in_refund'):
                    if rec.type == 'in_invoice':
