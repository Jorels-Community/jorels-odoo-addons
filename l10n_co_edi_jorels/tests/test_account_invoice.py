...
class TestAccountInvoice(TransactionCase):
    def setUp(self):
        ...
        # Adding sequence
        self.sequence = self.env['ir.sequence'].search([('name', '=', 'INV Sequence')]).write({
            'prefix': 'SETP',
            'padding': 4,
            'use_date_range': False,
            'number_next_actual': 1,
            'resolution_id': self.resolution_electronic_invoice.id
        })
    ...
    def test_create_edi_co_invoice(self):
        # Adding invoice
        ...
        invoice.write({
            'invoice_line_ids': [
                (0, 0, {
                    ...
                    'invoice_line_tax_ids': [(6, 0, [self.tax_excluido.id, self.tax_rtefte.id])],
                }),
                (0, 0, {
                    ...
                    'price_unit': 200.0,
                    'account_id': account_id,
                    'invoice_line_tax_ids': [(6, 0, [self.tax_iva.id, self.tax_rtefte.id])],
                })
            ]
        })

        # Check number of invoices
        invoices = self.env['account.invoice'].search([('partner_id', '=', self.partner.id)])
